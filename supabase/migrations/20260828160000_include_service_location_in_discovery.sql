/* Return the official service location in nearby discovery results. */
...
as $$
  with customer as (
    ...
    select t.trade, t.service_radius_km,
      coalesce(st_setsrid(st_makepoint(po.longitude, po.latitude), 4326)::geography, t.location) as point
    from public.technicians t
    left join lateral (
      select po.longitude, po.latitude from public.post_offices po
      where po.pincode = t.service_pincode and po.latitude is not null and po.longitude is not null
      order by po.name limit 1
    ) po on true
    where t.is_active and t.is_online and t.is_verified and t.status = 'ACTIVE'
  )
  select sl.trade, count(*)::bigint, min(st_distance(sl.point, customer.point) / 1000.0)::float8
  ...
  group by sl.trade order by sl.trade;
$$;
...
returns table (
  id uuid, name text, phone text, trade text, service_pincode text,
  service_location_name text, service_radius_km integer, is_active boolean,
  is_online boolean, latitude float8, longitude float8, distance_km float8
)
language sql stable security definer
set search_path = public, extensions
as $$
  with customer as (
    ...
    select t.id, t.name, t.phone, t.trade, t.service_pincode, t.service_radius_km,
      t.is_active, t.is_online, coalesce(po.name || ', ' || po.district, po.name) as service_location_name,
      coalesce(st_setsrid(st_makepoint(po.longitude, po.latitude), 4326)::geography, t.location) as point
    from public.technicians t
    left join lateral (
      select po.name, po.district, po.longitude, po.latitude from public.post_offices po
      where po.pincode = t.service_pincode and po.latitude is not null and po.longitude is not null
      order by po.name limit 1
    ) po on true
    where t.is_active and t.is_online and t.is_verified and t.status = 'ACTIVE'
  ...
  select sl.id, sl.name, sl.phone, sl.trade, sl.service_pincode, sl.service_location_name,
