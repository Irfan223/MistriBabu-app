...
interface TechnicianLoginModalProps {
  open: boolean;
  onClose: () => void;
  onJoinClick: () => void;
}
...
export default function TechnicianLoginModal({
  open,
  onClose,
  onJoinClick,
}: TechnicianLoginModalProps) {
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // res.data contains the body even on non-2xx; read it first for the real message
      ...
      // DEV MODE: surface OTP in UI so testing works without SMS
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // onAuthStateChange in App.tsx will detect the session and route to dashboard
      onClose();
    } catch (err) {
            ...
            <div>
              ...
              <div className="flex items-center gap-2 rounded-xl border border-slate-300 bg-slate-50 px-3 focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-200">
                ...
                <input
                  ...
                  placeholder="• • • • • •"
                  className="flex-1 bg-transparent py-3 text-center text-xl font-bold tracking-[0.5em] outline-none placeholder:text-slate-300 placeholder:tracking-widest"
                  autoFocus
                />
              </div>
